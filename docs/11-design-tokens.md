...
| Z-index             | `--z-base/content/transition/navbar/overlay/effect/modal/cursor/toast/debug` | `:root` (plain)                             | Now consumed — maps directly to the Global Layout's nine layers ([03](./03-folder-architecture.md#the-global-layout)). `modal`/`toast` remain reserved for content rendered inside a layer, not layers themselves. |
